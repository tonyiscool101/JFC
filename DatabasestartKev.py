...
with open("Employee_database.csv", mode="w", newline='') as f:
        ...
        for i in range(len(ID_list1)):
                if i<len(responselist): #Delete if statement after make responsiveness faster
                        writer.writerow({'ID': ID_list1[i][0],'Branch': ID_list1[i][1], "Jobtitle" : ID_list1[i][2],'Response': responselist[i], 'Evenness':evenesslist[i],'Succession String':succstringlist[i]})
                else:
